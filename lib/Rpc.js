...
Rpc._parseTxResult = function(info) {
  ...
  info.vout.forEach( function(o) {
    ...
    if (o.scriptPubKey.asm.indexOf("SSGEN") > -1 ) {
        ...
        if (config.network == 'exccdlivenet') {
          ...
          if (parseInt(hexLE, 16) == 4) {
            ...
            var lnsupport = 'LN: ';
            var sdiff = 'DCP0001: ';
            switch (parseInt(agendasLE, 16)&lnsupportMask) { case 0x08: lnsupport +='no';break;   case 0x10: lnsupport +='yes';break;   default: lnsupport+='abstain'; }
            switch (parseInt(agendasLE, 16)&sdiffMask) { case 0x02: sdiff +='no';break;  case 0x04: sdiff +='yes';break;   default: sdiff+='abstain'; }
            if (lnsupport != '') {
          ...
          } else if (parseInt(hexLE, 16) == 5) {
            var dcp23AgendaMask=0x06;
            var choice = 'DCP0002&3: ';
            switch (parseInt(agendasLE, 16)&dcp23AgendaMask) { case 0x02: choice += 'no';break;  case 0x04: choice +='yes';break;   default: choice+='abstain'; }
            if (choice != '') {
        ...
        } else if (config.network == 'exccdtestnet') {
...
Rpc.getBlock = function(hash, cb) {
  ...
  bitcoreRpc.getBlock(hash, function(err,info) {
    ...
    if (info.result.height) {
      ...
      // Set magic numbers from exccd/chaincfg/params.go
      ...
      if (info.result.height == 0) {
          ...
          if (config.network === 'exccdlivenet') {
            info.result.reward = 12144736;
          } else if (config.network === 'exccdtestnet') {
            info.result.reward = 5000000;
          }
      } else if ((info.result.height < 4096 && config.network === 'exccdlivenet') ||
                 (info.result.height < 768 && config.network === 'exccdtestnet')) {
          info.result.reward = Math.round(work) / bitcore.util.COIN;
      } else {
          info.result.reward = Math.round(work + stake) / bitcore.util.COIN;
      }
    }
